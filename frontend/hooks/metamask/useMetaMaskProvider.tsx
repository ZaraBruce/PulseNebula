...
function useMetaMaskInternal(): UseMetaMaskState {
  ...
  const accountsChangedListenerRef = useRef<AccountsChangedListenerFn | undefined>(
    undefined
  );
  ...
  useEffect(() => {
    ...
    if (next) {
      ...
      nextDisconnectListener = (error: ProviderRpcError) => {
        ...
        console.log(`[useMetaMask] disconnect`, error.code);
        _setCurrentProvider(undefined);
        _setChainId(undefined);
        _setAccounts(undefined);
      };
      ...
      const updateStatus = async () => {
        ...
        try {
          ...
          console.log(`[useMetaMask] unable to query status`, err);
        }
      };
