...
function useMetaMaskEthersSignerInternal(parameters: {
  ...
  const sameChain = useRef((nextChainId: number | undefined) => {
    return nextChainId === chainIdRef.current;
  });

  const sameSigner = useRef((nextSigner: ethers.JsonRpcSigner | undefined) => {
    return nextSigner === ethersSignerRef.current;
  });
  ...
  useEffect(() => {
    ...
    const browserProvider: ethers.BrowserProvider = new ethers.BrowserProvider(provider);
    let readonlyProvider: ethers.ContractRunner = browserProvider;
    const rpcUrl: string | undefined = initialMockChains?.[chainId];
    if (rpcUrl) {
      readonlyProvider = new ethers.JsonRpcProvider(rpcUrl);
    }

    const signer = new ethers.JsonRpcSigner(browserProvider, accounts[0]);
